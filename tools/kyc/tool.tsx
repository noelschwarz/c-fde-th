...
import { kycToolConfig } from "./config";
import { initialKycData, KYCCase } from "./data";

export const kycConnector = createMockConnector<KYCCase>({
  resource: kycToolConfig.connector.resource,
  initialData: initialKycData,
  getId: (item) => item.id,
});

export const kycRecordLabel = "Case";
...
export function renderKycCell(row: KYCCase, key: string): ReactNode | undefined {
  if (key === "status") {
    return (
